...
export async function searchDuckDuckGo(question: string, channel: string, user: string, app: SlackApp<{
    ...
    const topResultsText = await Promise.all(topResults.results.slice(0, 10).map(async (result: any) => {
